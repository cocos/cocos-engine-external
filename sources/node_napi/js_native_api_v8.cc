...
#define PRIVATE_KEY_LIST(V)                 \
    V(napi_type_tag, "nodec:napi:type_tag") \
    V(napi_wrapper, "nodec:napi:wrapper")

class PrivateEnv {
public:
    PrivateEnv(v8::Local<v8::Context> ctx);

#define V(FIELD_NAME, STRING_VALUE)              \
    inline v8::Local<v8::Private> FIELD_NAME() { \
        return FIELD_NAME##_.Get(isolate);       \
    }
    PRIVATE_KEY_LIST(V)
#undef V

    static PrivateEnv* fetch(v8::Local<v8::Context> ctx);

private:
    v8::Persistent<v8::Context> context;
    v8::Isolate*                isolate;
#define V(FIELD_NAME, STRING_VALUE) \
    v8::Eternal<v8::Private> FIELD_NAME##_;

    PRIVATE_KEY_LIST(V)
#undef V
};

PrivateEnv::PrivateEnv(v8::Local<v8::Context> ctx) {
    isolate = ctx->GetIsolate();
    context.Reset(isolate, ctx);
    //context.SetWeak();
#define V(FIELD, STRING)                                                                 \
    FIELD##_.Set(isolate, v8::Private::New(isolate,                                      \
...
int gEnvironmentTag = 0;

PrivateEnv* PrivateEnv::fetch(v8::Local<v8::Context> ctx) {
    PrivateEnv* env{nullptr};
    //assert(!privateEnvs.empty());
    void* t = ctx->GetAlignedPointerFromEmbedderData(kEnvironmentTag);
    void* p = ctx->GetAlignedPointerFromEmbedderData(kEnvironment);
    if (t != &gEnvironmentTag) {
        auto* e = new PrivateEnv(ctx);
        ctx->SetAlignedPointerInEmbedderData(kEnvironment, e);
        ctx->SetAlignedPointerInEmbedderData(kEnvironmentTag, &gEnvironmentTag);
        env = e;
    } else {
        env = reinterpret_cast<PrivateEnv*>(p);
    }
    assert(env);
    return env;
}

#define NAPI_PRIVATE_KEY(context, prop) \
    PrivateEnv::fetch(context)->napi_##prop()
...
#else
        _secondPassParameter, FinalizeCallback, v8::WeakCallbackType::kFinalizer);
#endif
...
v8::Isolate* napi_env_isolate(napi_env env) {
    return env->isolate;
}

napi_status
napi_define_class_with_parent(napi_env                                    env,
                              const char*                                 utf8name,
                              size_t                                      length,
                              napi_callback                               constructor,
                              void*                                       callback_data,
                              v8::Local<v8::FunctionTemplate>&            parent_tpl,
                              size_t                                      property_count,
                              const napi_property_descriptor*             properties,
                              napi_value*                                 result,
    ...
    *result                        = v8impl::JsValueFromV8LocalValue(
        scope.Escape(tpl->GetFunction(context).ToLocalChecked()));
    result_tpl.Reset(env->isolate, tpl);
...
napi_status napi_create_string_utf8(napi_env    env,
    ...
    *result = v8impl::JsValueFromV8LocalValue(str_maybe.ToLocalChecked());
    ...
    //    int newLength = str_maybe.ToLocalChecked()->Utf8Length(env->isolate);
    //    CHECK_EQ(newLength, length);
    //}
...
napi_status napi_reference_count(napi_env env, napi_ref ref, uint32_t* result) {
    // Omit NAPI_PREAMBLE and GET_RETURN_STATUS because V8 calls here cannot throw
    // JS exceptions.
    CHECK_ENV(env);
    CHECK_ARG(env, ref);
...
napi_status napi_run_script_with_filename(napi_env    env,
                                          napi_value  script,
                                          const char* filename,
                                          napi_value* result) {
    NAPI_PREAMBLE(env);
    ...
    v8::Local<v8::Context>     context   = env->context();
    v8::MaybeLocal<v8::String> originStr = v8::String::NewFromUtf8(env->isolate, filename == nullptr ? "(no filename)" : filename, v8::NewStringType::kNormal);
